      ...
      (while (re-search-forward "^;;; Code:\n[[:space:]\n]*(\\(package!\\|unpin!\\)[[:space:]\n]+\\([^[:space:]]+\\)\\b" nil t)
        (plist-put confpkg :package-statements
                   (nconc (plist-get confpkg :package-statements)
                          (list (match-string 2))))
        (message "Found package: %s" (match-string 2))
...
We can use the information collected in the execution of confpkg to create a
dependency map.

#+begin_src emacs-lisp
(let* ((doom-init ; ((:category modules...)...)
        (with-temp-buffer
                 ...
                 (mapcar
                  ...
                  (cdr (cl-reduce
                        (lambda (result new)
                          (cond
                           ((keywordp new)
                            (push (list new) result))
                           ((consp new)
                            (push (if (keywordp (car new))
                                      (if (consp (caddr new))
                                          (caaddr new)
                                        (caddr new))
                                    (car new))
                           ...
                           ((symbolp new)
                            (push new (car result))))
                          result)
                        (cdr (read (buffer-string)))
                        :initial-value (list nil)))))))))
       (doom-module-folder
        (or (bound-and-true-p doom-modules-dir)
            (and (file-exists-p (expand-file-name "~/.config/emacs/modules"))
                 (expand-file-name "~/.config/emacs/modules"))
            (and (file-exists-p (expand-file-name "~/.emacs/modules"))
                 (expand-file-name "~/.emacs/modules"))))
       (doom-module-packages ; ((:category (module "pkgs"...)...)...)
        (mapcar
         (lambda (category)
           (cons category
                 (cl-remove-if
                  ...
                  (mapcar
                   (lambda (module)
                     (let ((module-packages-path
                            (format "%s/%s/%s/packages.el"
                                    doom-module-folder
                                    (substring (symbol-name category) 1)
                                    module)))
                       (and (file-exists-p module-packages-path)
                            (cons module
                                  (with-temp-buffer
                                    (insert-file-contents module-packages-path)
                                    (goto-char (point-min))
                                    (let (pkgs)
                                      (while (re-search-forward
                                              "^[[:space:]]*(package![[:space:]\n]+\\([^[:space:]]+\\)\\b"
                                      ...
                                      pkgs))))))
                   (alist-get category doom-init)))))
         (mapcar #'car doom-init)))
       (doom-package-modules ; (("pkg" . (category . module))...)
        (let (pkg-cat-mod)
          (mapc
           (lambda (category)
             (mapc
              (lambda (module)
                (mapc
                 (lambda (pkg)
                   (push (cons pkg (cons (car category) (car module)))
                         pkg-cat-mod))
                 (cdr module)))
              (cdr category)))
           doom-module-packages)
          pkg-cat-mod))
       (doom-all-packages ; ("pkgs"...)
        (apply #'append
               (mapcar #'cdr
                       (apply #'append
                              (mapcar #'cdr
                                      doom-module-packages)))))
       (doom-all-packages-rx ; "\\_<\\(pkg1|pkg2...\\)\\_>"
        (regexp-opt doom-all-packages 'symbols))
       (confpkg-used-doom-pkgs ; (("config-pkg" "pkgs"...)...)
        (cl-remove-if-not
         #'cdr
         (mapcar
          (lambda (confpkg)
            (let ((pkgs-used) case-fold-search)
              (with-temp-buffer
                (insert-file-contents (plist-get confpkg :file))
                (goto-char (point-min))
                (while (re-search-forward doom-all-packages-rx nil t)
                  (push (match-string 0) pkgs-used)))
              (cons
               (plist-get confpkg :package)
               (cl-delete-duplicates pkgs-used :test #'equal))))
          confpkg--list)))
       (doom-configured-packages ; (("pkg" . (category . module))...)
        (mapcar
         (lambda (pkg)
           (assoc pkg doom-package-modules))
         (cl-delete-duplicates
          (apply #'append
                 (mapcar #'cdr confpkg-used-doom-pkgs))
          :test #'equal)))
       (dot-common-preamble
        "digraph {
  graph [bgcolor=\"transparent\"]
  overlap=\"prism\"
  node [shape=\"underline\", penwidth=\"2\", style=\"rounded,filled\", fillcolor=\"#efefef\", color=\"#c9c9c9\", fontcolor=\"#000000\", fontsize=\"18pt\", fontname=\"Alegreya Sans\"]
  edge [color=\"#aaaaaa\", penwidth=\"1.2\"]\n")
       )
  (with-temp-file "subconf/doom-modules.dot"
    (insert dot-common-preamble
            "\
  overlap_scaling=-40
  // Doom
  \"Doom\" [color=\"#a991f1\", shape=\"circle\", penwidth=\"8\", fontsize=\"52pt\", fontstyle=\"bold\"]
  node [color=\"#813d9c\"]\n")
    (dolist (category doom-init)
      (insert (format "  %1$S [fontsize=\"36pt\"]\n  %1$S -> \"Doom\"\n"
                      (symbol-name (car category))))
      (dolist (module (cdr category))
        (insert (format "  %1$S [label=%2$S]\n  %1$S -> %3$S\n"
                        (format "%s %s" (car category) module)
                        (symbol-name module)
                        (symbol-name (car category))))))
    (insert "// Packages
  node [fillcolor=\"#e69055\", penwidth=0, shape=\"circle\", fixedsize=true, width=0.1]\n")
    (let (drawn-pkgs (n 0) node)
      (dolist (category doom-module-packages)
        (dolist (module (cdr category))
          (dolist (pkg (cdr module))
            (setq node
                  (if (member pkg drawn-pkgs)
                      (format "%s-%d" pkg (cl-incf n))
                    (push pkg drawn-pkgs)
                    pkg))
            (insert (format "  %S [label=\"\"]\n  %S -> \"%s %s\"\n"
                            node node (car category) (car module)))))))
    (insert "}"))

  (with-temp-file "subconf/map.dot"
    (insert
     dot-common-preamble
     "overlap_scaling=-12
  // Config
  \"Config\" [color=\"#51afef\", shape=\"circle\", penwidth=\"5\", fontsize=\"26pt\"]
  // Subconf entries
  node [color=\"#4db5bd\"]\n"
            (mapconcat
             (lambda (confpkg)
            ...
            (mapconcat
             ...
             (cl-delete-duplicates
              (apply #'append
                     (mapcar
                      (lambda (confpkg)
                        (plist-get confpkg :package-statements))
                      confpkg--list))
              :test #'equal)
             "\n")
            "\n // Edges
  edge [color=\"#999999\", penwidth=\"1.5\"]\n")
    (dolist (confpkg confpkg--list)
      (dolist (req (plist-get confpkg :requires))
        (insert
         (format "  %S -> %S\n"
                 (if (symbolp req)
                   ...
                   req)
                 (plist-get confpkg :package))))
      (dolist (pkg (cl-delete-duplicates
                    (plist-get confpkg :package-statements)
                    :test #'equal))
        (insert
         (format "  %S -> %S\n"
    ...
    ;; Doom dependencies
    (insert "  // Doom
  \"Doom\" [color=\"#a991f1\", shape=\"circle\", penwidth=\"5\", fontsize=\"26pt\"]
  node [color=\"#813d9c\"]
  edge [color=\"#cccccc\"]\n")
    (dolist (category doom-init)
      (insert (format "  %1$S [fontsize=\"20pt\"]\n  %1$S -> \"Doom\"\n"
                      (symbol-name (car category))))
      (dolist (module (cdr category))
        (insert (format "  %1$S [label=%2$S]\n  %1$S -> %3$S\n"
                        (format "%s %s" (car category) module)
                        (symbol-name module)
                        (symbol-name (car category))))))
    (insert "  // Doom module packages
  node [color=\"#e69055\"]
  edge [color=\"#888888\"]\n")
    (dolist (doom-pkg doom-configured-packages)
      (insert (format "  %S -> \"%s %s\"\n"
                      (car doom-pkg)
                      (cadr doom-pkg)
                      (cddr doom-pkg))))
    (insert "  // Configuration of Doom-module-installed packages")
    (dolist (conf-pkgs confpkg-used-doom-pkgs)
      (dolist (pkg (cdr conf-pkgs))
        (insert (format "  %S -> %S\n"
                        pkg (car conf-pkgs)))))
    (insert "}")))
#+end_src
...
(defun +doom-dashboard-tweak (&optional _)
  (with-current-buffer (get-buffer +doom-dashboard-name)
    ...
    ;; (unless (memq 'doom-modeline-time (mapcar #'car face-remapping-alist))
...
#+begin_src emacs-lisp :tangle packages.el
(package! gptel :pin "906023095cd5ccb7160acf0c5b01926b06f3a7be")
#+end_src
...
#+call: confpkg("!Pkg Director")

Ergonomic interaction simulation

#+begin_src emacs-lisp :tangle packages.el
(package! director :pin "77d6f95aa65327e39ab64396b075a74ef2557c3b")
#+end_src

#+begin_src emacs-lisp
(use-package! director)
#+end_src
...
#+call: confpkg("Pkg ob-svgbob")

#+begin_src emacs-lisp :tangle packages.el
(package! ob-svgbob :recipe (:local-repo "lisp/ob-svgbob"))
#+end_src

#+begin_src emacs-lisp
(use-package! ob-svgbob :defer t)
#+end_src
...
#+call: confpkg("engraved-faces-tol-themes", prefix="", after="engrave-faces")

I'm rather a fan of Paul Tol's colour schemes, and so it would be nice to be
able to make Tol-flavoured presentations. With ox-chameleon we can do this by
defining some Tol engrave faces themes.
...
(add-to-list
 ...
 '(tol-muted
   (default                             :short "default"             :slug "D"   :family "JetBrains Mono" :foreground "#222222" :background "#fff6cd" :slant normal :weight normal)
   (variable-pitch                      :short "var-pitch"           :slug "vp"  :family "Overpass"       :height 176)
   (shadow                              :short "shadow"              :slug "h"   :foreground "#777777")
   (success                             :short "success"             :slug "sc"  :foreground "#117733")
   (warning                             :short "warning"             :slug "w"   :foreground "#999933")
   (error                               :short "error"               :slug "e"   :foreground "#CC6677")
   (link                                :short "link"                :slug "l"   :foreground "#332288"    :weight bold)
   (link-visited                        :short "link"                :slug "lv"  :foreground "#882255"    :weight bold)
   (highlight                           :short "link"                :slug "hi"  :foreground "#f2f2f2"    :background "#332288")
   (font-lock-comment-face              :short "fl-comment"          :slug "c"   :foreground "#555555")
   (font-lock-comment-delimiter-face    :short "fl-comment-delim"    :slug "cd"  :foreground "#555555")
   (font-lock-string-face               :short "fl-string"           :slug "s"   :foreground "#117733")
   (font-lock-doc-face                  :short "fl-doc"              :slug "d"   :foreground "#444444"    :slant italic)
   (font-lock-doc-markup-face           :short "fl-doc-markup"       :slug "m"   :foreground "#882255")
   (font-lock-keyword-face              :short "fl-keyword"          :slug "k"   :foreground "#AA4499")
   (font-lock-builtin-face              :short "fl-builtin"          :slug "b"   :foreground "#882255")
   (font-lock-function-name-face        :short "fl-function"         :slug "f"   :foreground "#332288")
   (font-lock-variable-name-face        :short "fl-variable"         :slug "v"   :foreground "#44AA99")
   (font-lock-type-face                 :short "fl-type"             :slug "t"   :foreground "#999933")
   (font-lock-constant-face             :short "fl-constant"         :slug "o"   :foreground "#332288")
   (font-lock-warning-face              :short "fl-warning"          :slug "wr"  :foreground "#999933")
   (font-lock-negation-char-face        :short "fl-neg-char"         :slug "nc"  :weight bold)
   (font-lock-preprocessor-face         :short "fl-preprocessor"     :slug "pp"  :weight bold)
   ...
   (org-block                           :short "org-block"           :slug "ob"  :background "#f5f5f5")
   (org-block-begin-line                :short "org-block-begin"     :slug "obb" :foreground "#555555"    :background "#f5f5f5")
   (org-block-end-line                  :short "org-block-end"       :slug "obe" :foreground "#555555"    :background "#f5f5f5")
   (outline-1                           :short "outline-1"           :slug "Oa"  :foreground "#44AA99"    :weight extra-bold    :height 1.25)
   (outline-2                           :short "outline-2"           :slug "Ob"  :foreground "#AA4499"    :weight bold          :height 1.15)
   (outline-3                           :short "outline-3"           :slug "Oc"  :foreground "#332288"    :weight bold          :height 1.12)
   (outline-4                           :short "outline-4"           :slug "Od"  :foreground "#44AA99"    :weight semi-bold     :height 1.09)
   (outline-5                           :short "outline-5"           :slug "Oe"  :foreground "#AA4499"    :weight semi-bold     :height 1.06)
   (outline-6                           :short "outline-6"           :slug "Of"  :foreground "#332288"    :weight semi-bold     :height 1.03)
   (outline-7                           :short "outline-7"           :slug "Og"  :foreground "#44AA99"    :weight bold)
   (outline-8                           :short "outline-8"           :slug "Oh"  :foreground "#AA4499"    :weight semi-bold)
   (highlight-numbers-number            :short "hl-number"           :slug "hn"  :foreground "#882255"    :weight bold)
   (highlight-quoted-quote              :short "hl-qquote"           :slug "hq")
   (highlight-quoted-symbol             :short "hl-qsymbol"          :slug "hs"  :foreground "#999933")
   (rainbow-delimiters-depth-1-face     :short "rd-1"                :slug "rda" :foreground "#AA4499")
   (rainbow-delimiters-depth-2-face     :short "rd-2"                :slug "rdb" :foreground "#332288")
   (rainbow-delimiters-depth-3-face     :short "rd-3"                :slug "rdc" :foreground "#117733")
   (rainbow-delimiters-depth-4-face     :short "rd-4"                :slug "rdd" :foreground "#882255")
   (rainbow-delimiters-depth-5-face     :short "rd-5"                :slug "rde" :foreground "#44AA99")
   (rainbow-delimiters-depth-6-face     :short "rd-6"                :slug "rdf" :foreground "#999933")
   (rainbow-delimiters-depth-7-face     :short "rd-7"                :slug "rdg" :foreground "#AA4499")
   (rainbow-delimiters-depth-8-face     :short "rd-8"                :slug "rdh" :foreground "#332288")
   (rainbow-delimiters-depth-9-face     :short "rd-9"                :slug "rdi" :foreground "#117733")
   (org-document-title                  :short "org-title"           :slug "ot"  :foreground "#44AA99")
   (org-document-info                   :short "org-docinfo"         :slug "odi" :foreground "#332288")
   (org-link                            :short "org-link"            :slug "ol"  :foreground "#44AA99")
   (org-list-dt                         :short "org-list"            :slug "oli" :foreground "#88CCEE")
   (org-code                            :short "org-code"            :slug "oc"  :foreground "#AA4499")
   (org-verbatim                        :short "org-verbatim"        :slug "ov"  :foreground "#332288")
   (org-cite                            :short "org-cite"            :slug "ok"  :foregroud  "#999933")))
#+end_src
...
(add-to-list
 'engrave-faces-themes
 '(org-site
   (default                             :short "default"             :slug "D"   :foreground "#2e3436" :background "#fafafa" :slant normal :weight normal)
   (shadow                              :short "shadow"              :slug "h"   :foreground "#777777")
   (success                             :short "success"             :slug "sc"  :foreground "#2d9574")
   (warning                             :short "warning"             :slug "w"   :foreground "#dc752f")
   (error                               :short "error"               :slug "e"   :foreground "#ba2f59")
   (link                                :short "link"                :slug "l"   :foreground "#3a81c3" :weight bold)
   (link-visited                        :short "link"                :slug "lv"  :foreground "#715ab1" :weight bold)
   (highlight                           :short "link"                :slug "hi"  :foreground "#dddddd" :background "#77aa99")
   (font-lock-comment-face              :short "fl-comment"          :slug "c"   :foreground "#555555")
   (font-lock-comment-delimiter-face    :short "fl-comment-delim"    :slug "cd"  :foreground "#555555")
   (font-lock-string-face               :short "fl-string"           :slug "s"   :foreground "#2d9574")
   (font-lock-doc-face                  :short "fl-doc"              :slug "d"   :foreground "#444444" :slant italic)
   (font-lock-doc-markup-face           :short "fl-doc-markup"       :slug "m"   :foreground "#aa8b77")
   (font-lock-keyword-face              :short "fl-keyword"          :slug "k"   :foreground "#2aa1ae")
   (font-lock-builtin-face              :short "fl-builtin"          :slug "b"   :foreground "#806656")
   (font-lock-function-name-face        :short "fl-function"         :slug "f"   :foreground "#3a81c3")
   (font-lock-variable-name-face        :short "fl-variable"         :slug "v"   :foreground "#77aa99")
   (font-lock-type-face                 :short "fl-type"             :slug "t"   :foreground "#715ab1")
   (font-lock-constant-face             :short "fl-constant"         :slug "o"   :foreground "#587e72")
   (font-lock-warning-face              :short "fl-warning"          :slug "wr"  :foreground "#dc752f")
   (font-lock-negation-char-face        :short "fl-neg-char"         :slug "nc"  :weight bold)
   (font-lock-preprocessor-face         :short "fl-preprocessor"     :slug "pp"  :weight bold)
   (font-lock-regexp-grouping-construct :short "fl-regexp"           :slug "rc"  :weight bold)
   (font-lock-regexp-grouping-backslash :short "fl-regexp-backslash" :slug "rb"  :weight bold)
   ...
   (org-block-begin-line                :short "org-block-begin"     :slug "obb" :foreground "#555555" :background "#f5f5f5")
   (org-block-end-line                  :short "org-block-end"       :slug "obe" :foreground "#555555" :background "#f5f5f5")
   (outline-1                           :short "outline-1"           :slug "Oa"  :foreground "#587e72" :weight extra-bold    :height 1.25)
   (outline-2                           :short "outline-2"           :slug "Ob"  :foreground "#77aa99" :weight bold          :height 1.15)
   (outline-3                           :short "outline-3"           :slug "Oc"  :foreground "#587e72" :weight bold          :height 1.12)
   (outline-4                           :short "outline-4"           :slug "Od"  :foreground "#806656" :weight semi-bold     :height 1.09)
   (outline-5                           :short "outline-5"           :slug "Oe"  :foreground "#587e72" :weight semi-bold     :height 1.06)
   (outline-6                           :short "outline-6"           :slug "Of"  :foreground "#77aa99" :weight semi-bold     :height 1.03)
   (outline-7                           :short "outline-7"           :slug "Og"  :foreground "#806656" :weight bold)
   (outline-8                           :short "outline-8"           :slug "Oh"  :foreground "#587e72" :weight semi-bold)
   (highlight-numbers-number            :short "hl-number"           :slug "hn"  :foreground "#dc752f" :weight bold)
   (highlight-quoted-quote              :short "hl-qquote"           :slug "hq")
   (highlight-quoted-symbol             :short "hl-qsymbol"          :slug "hs"  :foreground "#806656")
   (rainbow-delimiters-depth-1-face     :short "rd-1"                :slug "rda" :foreground "#715ab1")
   (rainbow-delimiters-depth-2-face     :short "rd-2"                :slug "rdb" :foreground "#2aa1ae")
   (rainbow-delimiters-depth-3-face     :short "rd-3"                :slug "rdc" :foreground "#806656")
   (rainbow-delimiters-depth-4-face     :short "rd-4"                :slug "rdd" :foreground "#715ab1")
   (rainbow-delimiters-depth-5-face     :short "rd-5"                :slug "rde" :foreground "#2aa1ae")
   (rainbow-delimiters-depth-6-face     :short "rd-6"                :slug "rdf" :foreground "#806656")
   (rainbow-delimiters-depth-7-face     :short "rd-7"                :slug "rdg" :foreground "#715ab1")
   (rainbow-delimiters-depth-8-face     :short "rd-8"                :slug "rdh" :foreground "#2aa1ae")
   (rainbow-delimiters-depth-9-face     :short "rd-9"                :slug "rdi" :foreground "#806656")
   (org-document-title                  :short "org-title"           :slug "ot"  :foreground "#587e72")
   (org-document-info                   :short "org-docinfo"         :slug "odi" :foreground "#806656")
   (org-link                            :short "org-link"            :slug "ol"  :foreground "#587e72")
   (org-list-dt                         :short "org-list"            :slug "oli" :foreground "#587e72")
   (org-code                            :short "org-code"            :slug "oc"  :foreground "#2aa1ae")
   (org-verbatim                        :short "org-verbatim"        :slug "ov"  :foreground "#715ab1")
   (org-cite                            :short "org-cite"            :slug "ok"  :foregroud  "#806656")))
#+end_src
...
#+call: confpkg("Lang Janet")

Unfortunately there is no Janet module in Doom, so we'll have to grab the
essentials ourself.

#+begin_src emacs-lisp :tangle packages.el
(package! janet-mode :pin "9e3254a0249d720d5fa5603f1f8c3ed0612695af")
(package! flycheck-janet :recipe (:host github :repo "sogaiu/flycheck-janet")
  :pin "687132b1b6c10ac4338186214e3eaa3fa100f365")
#+end_src

Now we just need to set these up.
